import { PeerConnection } from '../lib/index.mjs';
import RTCSessionDescription from './RTCSessionDescription.mjs';
import RTCDataChannel from './RTCDataChannel.mjs';
import RTCIceCandidate from './RTCIceCandidate.mjs';
import { RTCDataChannelEvent, RTCPeerConnectionIceEvent } from './Events.mjs';
import RTCSctpTransport from './RTCSctpTransport.mjs';
import { NotFoundError, SyntaxError, InvalidAccessError, OperationError, InvalidStateError } from './Exception.mjs';
...
class RTCPeerConnection extends EventTarget {
  constructor(config = { iceServers: [], iceTransportPolicy: "all" }) {
    ...
    try {
      ...
      __privateSet(this, _peerConnection, config?.peerConnection ?? new PeerConnection(peerIdentity, {
        ...config,
      ...
      if (!error || !error.message) throw new NotFoundError("Unknown error");
    ...
    __privateGet(this, _peerConnection).onLocalDescription((sdp, type) => {
      if (type === "offer") {
        __privateGet(this, _localOffer).resolve(new RTCSessionDescription({ sdp, type }));
      }
      if (type === "answer") {
        __privateGet(this, _localAnswer).resolve(new RTCSessionDescription({ sdp, type }));
      }
    });
    __privateGet(this, _peerConnection).onLocalCandidate((candidate, sdpMid) => {
      ...
      __privateGet(this, _localCandidates).push(new RTCIceCandidate({ candidate, sdpMid }));
    ...
    __privateSet(this, _sctp, new RTCSctpTransport({
      pc: this
    }));
  }
    ...
    if (Array.isArray(config?.iceServers)) {
      for (let i = 0; i < config.iceServers.length; i++) {
        ...
        if (config.iceServers[i].urls?.some((url) => url == ""))
          throw new SyntaxError("IceServers urls cannot be empty");
        if (config.iceServers[i].urls?.some((url) => {
          ...
          throw new SyntaxError("IceServers urls wrong format");
  ...
  get currentLocalDescription() {
    return new RTCSessionDescription(__privateGet(this, _peerConnection).localDescription());
  }
  get currentRemoteDescription() {
    return new RTCSessionDescription(__privateGet(this, _peerConnection).remoteDescription());
  }
...
//# sourceMappingURL=RTCPeerConnection.mjs.map
